...
namespace PlantNanny.API.Repositories
{
    public class PlantRepository : IPlantRepository
    {
        ...
        {
            return await context.Plants.Where(c => c.Name.ToLower() == _Name.ToLower()).ToListAsync();
        }

        // Returns all plants with matching Scientific Name
        public async Task<IEnumerable<Plant>> GetbySciName(string _SciName)
        {
            return await context.Plants.Where(c => c.Sci_Name.ToLower() == _SciName.ToLower()).ToListAsync();
        }

        // Returns all plants with matching Region
        public async Task<IEnumerable<Plant>> GetbyRegion(string _Region)
        {
            return await context.Plants.Where(c => c.Region.ToLower() == _Region.ToLower()).ToListAsync();
        }
        ...
        {
            return await context.Plants.Where(c => c.Region.ToLower() == _Region.ToLower() 
            && c.Name.ToLower() == _Name.ToLower()).ToListAsync();
        }
        ...
        {
            return await context.Plants.Where(c => c.Region.ToLower() == _Region.ToLower()
            && c.Sci_Name.ToLower() == _SciName.ToLower()).ToListAsync();
        }

        public async Task<IEnumerable<Plant>> GetbySearch(string? _Name = null, string? _SciName = null, string? _Region = null)
        {
            if(string.IsNullOrEmpty(_Name) && string.IsNullOrEmpty(_SciName) && string.IsNullOrEmpty(_Region))
            {
                return await Get();
            }
            else if (string.IsNullOrEmpty(_Region))
            {
                return await context.Plants.Where(c => 
                c.Name.ToLower() == _Name.ToLower() && 
                c.Sci_Name.ToLower() == _SciName.ToLower()).ToListAsync();
            }
            else if (string.IsNullOrEmpty(_SciName))
            {
                return await context.Plants.Where(c =>
                c.Name.ToLower() == _Name.ToLower() &&
                c.Region.ToLower() == _Region.ToLower()).ToListAsync();
            }
            else if (string.IsNullOrEmpty(_Name))
            {
                return await context.Plants.Where(c =>
                c.Region.ToLower() == _Region.ToLower() &&
                c.Sci_Name.ToLower() == _SciName.ToLower()).ToListAsync();
            }
            else// if (string.IsNullOrEmpty(_Region))
            {
                return await context.Plants.Where(c =>
                c.Name.ToLower() == _Name.ToLower() &&
                c.Sci_Name.ToLower() == _SciName.ToLower() &&
                c.Region.ToLower() == _Region.ToLower()).ToListAsync();
            }
        }
